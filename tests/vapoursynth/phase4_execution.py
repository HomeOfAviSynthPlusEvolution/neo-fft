...
def main():
    ...
    for bt in (-1,0,1,3,5):
        ...
        a=c.neo_fft.FFT3D(src,**kw)
        b=c.neo_fft.FFT3D(src,mt=True,ncpu=1<<40,measure=True,fft_backend=b'unknown\x00backend',**kw)
        order=(6,0,2,4)
    ...
    for T in (1,3):
        for mode in (0,1):
            for dither in (0,1,4):
                ...
                b=c.neo_fft.DFTTest(src,threads=1<<40,fft_threads=-(1<<40),fft_backend='gpu',**kw)
                order=(6,2,0,6)
                expected=[a.get_frame(n) for n in order]
                pending=[b.get_frame_async(n) for n in order]
                for n,fa,future in zip(order,expected,pending):
    ...
    for threads in (-(1<<40),-2147483648,-1,0,1,3,16,17,2147483647,1<<40):
    ...
    print('Phase 4 execution: host concurrency and ignored legacy arguments invariant; opt aliases; effective PocketFFT workers 1')
