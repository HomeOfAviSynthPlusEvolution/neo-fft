  ...
  try {
    ...
    for(int T:{3,4,5})for(int S:{8,12,16,32}) {
      ...
      for(auto profile:{FftProfile::scalar,FftProfile::sse2,FftProfile::avx2,FftProfile::avx512,FftProfile::native}) {
        ...
        CHECK(!transform.try_inverse_slice(0,nullptr,0,0,nullptr,0));
        CHECK(!transform.try_inverse_slice(0,nullptr,1,0,nullptr,0));
        CHECK(!transform.try_inverse_slice(-1,nullptr,2,0,nullptr,0));
        CHECK(!transform.try_inverse_slice(T,nullptr,2,0,nullptr,0));
        for(int count:{0,1,2,7,8,9}) {
          ...
          // slice index and scale; non-target slices and inactive volumes are guards.
          for(int b=0;b<count;++b) {
            const auto& oracle=b%2 ? oracle1 : oracle0;
          ...
          for(int slice=0;slice<T;++slice) {
            const auto center=std::size_t(slice)*S*S;
