...
Specification: DFT-VS-004. Extends [phase-3 interface](../../phase-3/dfttest/plugin.md). The user-directed interface revision removes fft_threads; supplying it fails creation, including historical defaults. Remaining parameters retain their relative registration order/defaults, and the now-active omitted dither seed has deterministic semantics.
