...
kratio^2 must be finite when Kalman is the effective mode. For uniform/analytic models validate the resulting motion thresholds at creation; sampled-model thresholds are validated during model construction. For effective Kalman, positive pfactor selects sampled mode but its magnitude does not scale sampled power: use the unscaled measured spatial power before the Kalman floor. This overrides the phase-3 Wiener strength rule only for Kalman; preview and bt=1..5 retain their inherited behavior. Do not calculate Wiener beta gains or degrid corrections inside the recurrence. Build enhancement constants only for enabled enhancement; enhancement's separate degrid path remains inherited.
