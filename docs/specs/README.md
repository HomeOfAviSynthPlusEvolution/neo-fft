...
Current execution policy, including the subsequent KernelInfo cleanup, is defined in [RUN-004](phase-4/execution.md): both filters execute inline on the calling host thread, while host frame concurrency remains supported. DFTTest threads is reserved and has no execution or retention effect. FFT3D mt/ncpu/measure and DFTTest fft_threads are removed; Both filter input signatures also remove fft_backend; PocketFFT is fixed internally. KernelInfo retains its fft_backend diagnostic but no longer reports fft_threads. Earlier phase signatures and admission rules are historical baselines, superseded by these explicit revisions.
