...
inline void fft16_8way_avx2(__m256* r, __m256* i) noexcept {
  ...
  #define BUTTERFLY8(x0_r, x0_i, x1_r, x1_i, x2_r, x2_i, x3_r, x3_i, \
    ...
    __m256 u1r = x1_r, u1i = x1_i; \
    x1_r = _mm256_add_ps(u1r, v1r); x1_i = _mm256_add_ps(u1i, v1i); \
    x5_r = _mm256_sub_ps(u1r, v1r); x5_i = _mm256_sub_ps(u1i, v1i); \
    __m256 v2r, v2i; \
    MUL_J(v2r, v2i, x6_r, x6_i); \
    __m256 u2r = x2_r, u2i = x2_i; \
    x2_r = _mm256_add_ps(u2r, v2r); x2_i = _mm256_add_ps(u2i, v2i); \
    x6_r = _mm256_sub_ps(u2r, v2r); x6_i = _mm256_sub_ps(u2i, v2i); \
    __m256 v3r, v3i; \
    CMUL_SCALAR(v3r, v3i, x7_r, x7_i, W16_R[6], W16_I[6]); \
    __m256 u3r = x3_r, u3i = x3_i; \
    x3_r = _mm256_add_ps(u3r, v3r); x3_i = _mm256_add_ps(u3i, v3i); \
    x7_r = _mm256_sub_ps(u3r, v3r); x7_i = _mm256_sub_ps(u3i, v3i); \
  }
  ...
  for (int k = 1; k < 8; ++k) {
    __m256 ak_r, ak_i, bk_r, bk_i;
    if (k == 1) { ak_r = a1_r; ak_i = a1_i; bk_r = a9_r; bk_i = a9_i; }
    else if (k == 2) { ak_r = a2_r; ak_i = a2_i; bk_r = a10_r; bk_i = a10_i; }
    else if (k == 3) { ak_r = a3_r; ak_i = a3_i; bk_r = a11_r; bk_i = a11_i; }
    else if (k == 4) { ak_r = a4_r; ak_i = a4_i; bk_r = a12_r; bk_i = a12_i; }
    else if (k == 5) { ak_r = a5_r; ak_i = a5_i; bk_r = a13_r; bk_i = a13_i; }
    else if (k == 6) { ak_r = a6_r; ak_i = a6_i; bk_r = a14_r; bk_i = a14_i; }
    else { ak_r = a7_r; ak_i = a7_i; bk_r = a15_r; bk_i = a15_i; }

    __m256 vr, vi;
    if (k == 4) {
      MUL_J(vr, vi, bk_r, bk_i);
    } else {
      CMUL_SCALAR(vr, vi, bk_r, bk_i, W16_R[k], W16_I[k]);
    }
    r[k]     = _mm256_add_ps(ak_r, vr);
    i[k]     = _mm256_add_ps(ak_i, vi);
    r[k + 8] = _mm256_sub_ps(ak_r, vr);
    i[k + 8] = _mm256_sub_ps(ak_i, vi);
  }
...
inline void transpose8x8_ps(__m256& r0, __m256& r1, __m256& r2, __m256& r3,
                            __m256& r4, __m256& r5, __m256& r6, __m256& r7) noexcept {
...
inline void batch_r2c_avx2(std::size_t batch, const float* in, std::size_t in_dist,
  ...
  for (std::size_t b = 0; b < batch; ++b) {
    ...
    // 1. Horizontal pass directly to blk_out
    for (int g = 0; g < 2; ++g) {
      const int y_base = g * 8;
      __m256 left[8], right[8];
      for (int y = 0; y < 8; ++y) {
        const float* src = blk_in + (y_base + y) * in_stride;
        left[y]  = _mm256_loadu_ps(src);
        right[y] = _mm256_loadu_ps(src + 8);
      }
      transpose8x8_ps(left[0], left[1], left[2], left[3], left[4], left[5], left[6], left[7]);
      transpose8x8_ps(right[0], right[1], right[2], right[3], right[4], right[5], right[6], right[7]);

      __m256 r[16], i[16];
      for (int x = 0; x < 8; ++x) {
        r[x]     = left[x];
        r[x + 8] = right[x];
        i[x]     = _mm256_setzero_ps();
        i[x + 8] = _mm256_setzero_ps();
      }
      ...
      for (int kx = 0; kx < 9; ++kx) {
        ...
        for (int y = 0; y < 8; ++y) {
          blk_out[(y_base + y) * out_stride + kx] = cfloat(tr[y], ti[y]);
        }
      }
    }

    // 2. Vertical pass in-place on blk_out
    {
      __m256 r[16], i[16];
      for (int y = 0; y < 16; ++y) {
        const cfloat* src = blk_out + y * out_stride;
        alignas(32) float tr[8], ti[8];
    ...
    }
    {
      ...
      for (int ky = 0; ky < 16; ++ky) {
        ...
        _mm256_store_ps(tr, r[ky]);
        _mm256_store_ps(ti, i[ky]);
        blk_out[ky * out_stride + 8] = cfloat(tr[0], ti[0]);
      }
    }
  }
}
...
inline void batch_c2r_avx2(std::size_t batch, const cfloat* in, std::size_t in_dist,
  ...
  alignas(64) float inter_r[16][9];
  alignas(64) float inter_i[16][9];
  ...
  for (std::size_t b = 0; b < batch; ++b) {
    ...
    // 1. Vertical 1D IFFT to inter buffer
    {
      __m256 r[16], i[16];
      for (int y = 0; y < 16; ++y) {
...
inline void batch_fft16x16_r2c(std::size_t batch, const float* in, std::size_t in_dist,
                               std::size_t in_stride, cfloat* out, std::size_t out_dist,
                               std::size_t out_stride) noexcept {
#if defined(__AVX2__)
  detail_16x16::batch_r2c_avx2(batch, in, in_dist, in_stride, out, out_dist, out_stride);
#else
  (void)batch;
  (void)in;
  (void)in_dist;
...
inline void batch_fft16x16_c2r(std::size_t batch, const cfloat* in, std::size_t in_dist,
                               std::size_t in_stride, float* out, std::size_t out_dist,
                               std::size_t out_stride, float fct) noexcept {
#if defined(__AVX2__)
  detail_16x16::batch_c2r_avx2(batch, in, in_dist, in_stride, out, out_dist, out_stride, fct);
#else
  (void)batch;
  (void)in;
  (void)in_dist;
