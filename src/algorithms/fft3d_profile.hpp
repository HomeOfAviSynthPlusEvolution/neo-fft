...
std::vector<float> fft3d_profile(int width, int height, const std::array<float, 4>& sigmas);
struct EnhancementTables {
  std::vector<float> sharpen, halo;
  float a = 0, b = 0, c = 0;
  Enhancement view(const EnhancementConfig& config) const;
};
EnhancementTables enhancement_tables(int width, int height, float format_scale, const EnhancementConfig& c);
} // namespace neo_fft
